...
class TrainerSAREnGAN:
    ...
    def __init__(self, device):
        # 感知特征
        self.perceptual_extractor = FeatureVGG19().to(device)
    ...
    def train_generator(self, optimizer_G, img_gen, img_hr):
        optimizer_G.zero_grad()
        # pixel loss
        loss_pixel = self.L1Loss(img_gen, img_hr)

        feature_real = self.perceptual_extractor(img_hr)
        feature_gen = self.perceptual_extractor(img_gen).detach()
        loss_perceptual = self.MSELoss(feature_real, feature_gen)

        lossSSIM = self.ssim(img_gen, img_hr)

        # Total generator loss
        loss_G = loss_perceptual + lossSSIM + loss_pixel
        loss_G.backward()

        optimizer_G.step()

        return loss_G, loss_perceptual, lossSSIM, loss_pixel
    ...
    def train_discriminator(self, optimizer_D, discriminator, img_gen, img_hr, real_labels, fake_labels):
        ...
        loss_real = self.MSELoss(real_score, real_labels)
        loss_fake = self.MSELoss(fake_score, fake_labels)
...
def train(opt):
    # 创建文件夹
    save_folder_image = os.path.join(opt.save_folder, r"SAREnGAN/images")
    save_folder_model = os.path.join(opt.save_folder, r"SAREnGAN/models")
    ...
    dataset = ImageDatasetPair(dataset_high, dataset_low, is_Normalize=False)
    ...
    # 定义训练
    trainer = TrainerSAREnGAN(device)
    ...
    for epoch in range(trained_epoch + 1, n_epochs + trained_epoch + 1):
        ...
        for batch_idx, images_hl in tqdm(enumerate(train_dataloader), desc=f'Training Epoch {epoch}',
            ...
            loss_G, loss_perceptual, lossSSIM, loss_pixel = trainer.train_generator(optimizer_G, img_gen, img_hr)
            ...
            ##########################
            loss_D, loss_real,loss_fake = trainer.train_discriminator(optimizer_D, discriminator, img_gen, img_hr, real_labels, fake_labels)
        ...
        with torch.no_grad():
            ...
            for batch_idx, images_hl in tqdm(enumerate(val_dataloader), desc=f'Validate Epoch {epoch}',
                ...
                loss_G, loss_perceptual, lossSSIM, loss_pixel = trainer.get_generate_loss(discriminator, img_gen, img_hr, real_labels)

                val_psnr = calc_psnr(img_gen.detach(), img_hr)

                loss_D, loss_real,loss_fake = trainer.get_disc_loss(discriminator, img_gen, img_hr, real_labels, fake_labels)
